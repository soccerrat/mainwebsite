...
import { initializeGoogleAnalytics, trackEvent, trackPageView } from './analytics.js'
...
function ContactPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (result.success) { trackEvent('contact_form_submitted', { service: String(new FormData(form).get('service') || 'not_selected') }); form.reset(); setFormStatus({ type: 'success', message: 'Your request was sent. VeritaGrid will follow up soon.' }) }
...
function App() {
  ...
  return <div className="min-h-screen bg-cloud text-ink"><Header page={page} navigate={navigate} /><main key={page} className="route-enter">{content}</main><Footer navigate={navigate} /><Analytics /></div>
}
